...
def _resolve_image_paths(image_args: list[str] | None, *, provider: str) -> list[Path]:
    paths: list[Path] = []
    for image_arg in image_args or []:
        path = _resolve_image_path(image_arg, provider=provider)
        if path is not None:
            paths.append(path)
    return paths
...
def main(
    ...
    *,
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Optional reference image to augment --prompt. Repeat for multiple images.",
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    try:
        image_paths = _resolve_image_paths(args.image, provider=provider)
    except Exception as exc:
    ...
    user_content = _build_initial_user_content(prompt_with_qc, image_paths=image_paths)
