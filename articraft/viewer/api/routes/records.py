...
from viewer.api.video_export import (
    export_viewer_mp4,
    record_viewer_url,
    staging_viewer_url,
)
...
async def export_record_mp4(
    record_id: str,
    payload: ExportMp4Request,
    store: ViewerStoreDep,
    resolver: FileResolverDep,
) -> ExportMp4Response:
    _validate_record_id(record_id)
    record_dir = resolver.resolve_record_folder(record_id)
    output_path = record_dir / "exports" / "motion.mp4"
    file_url = f"/api/records/{quote(record_id)}/files/exports/motion.mp4"

    try:
        result = await export_viewer_mp4(
            repo_root=store.repo.root,
            viewer_url=payload.viewer_url or record_viewer_url(record_id),
...
@router.post(
    "/api/staging/{run_id}/{record_id}/export-mp4",
    response_model=ExportStagingMp4Response,
)
async def export_staging_mp4(
    run_id: str,
    record_id: str,
    payload: ExportMp4Request,
    store: ViewerStoreDep,
    ...
    output_path = staging_dir / "exports" / "motion.mp4"
    file_url = f"/api/staging/{quote(run_id)}/{quote(record_id)}/files/exports/motion.mp4"

    try:
        result = await export_viewer_mp4(
            repo_root=store.repo.root,
            viewer_url=payload.viewer_url or staging_viewer_url(run_id, record_id),
            output_path=output_path,
            file_url=file_url,
            duration=payload.duration,
            fps=payload.fps,
    ...
    return ExportStagingMp4Response(
        status="exported",
        run_id=run_id,
        record_id=record_id,
        path=str(result.output_path),
