...
const ANIMATION_SPEED_QUERY_PARAM = "animation_speed";
const EXPORT_CANVAS_QUERY_PARAM = "export_canvas";
...
type ExportJointInfo = {
  name: string;
  type: UrdfJoint["type"];
};

type ArticraftExportController = {
  ready: boolean;
  joints: ExportJointInfo[];
  setSingleJointPhase: (jointName: string, phase: number) => void;
  setSequentialJointPhase: (jointIndex: number, phase: number) => void;
  reset: () => void;
};

declare global {
  interface Window {
    __ARTICRAFT_EXPORT__?: ArticraftExportController;
  }
}
...
function readAnimationSpeedFromUrl(): number {
  if (typeof window === "undefined") {
    return 1;
  }

  try {
    const raw = new URLSearchParams(window.location.search).get(ANIMATION_SPEED_QUERY_PARAM);
    if (!raw) {
      return 1;
    }
    ...
    return THREE.MathUtils.clamp(parsed, 0.05, 4);
  } catch {
    return 1;
  }
}

function readExportCanvasModeFromUrl(): boolean {
  if (typeof window === "undefined") {
    return false;
  }

  try {
    return new URLSearchParams(window.location.search).get(EXPORT_CANVAS_QUERY_PARAM) === "1";
  } catch {
    return false;
  }
}
...
function exportJointValue(joint: UrdfJoint, phase: number): number {
  const normalizedPhase = THREE.MathUtils.clamp(phase, 0, 1);
  ...
  if (hasRange) {
    const normalized = 0.5 - (0.5 * Math.cos(normalizedPhase * Math.PI));
    return THREE.MathUtils.lerp(lower, upper, normalized);
  }

  const eased = 0.5 - (0.5 * Math.cos(normalizedPhase * Math.PI));
  if (joint.type === "prismatic") {
    return eased * 0.12;
  }
  return eased * (Math.PI / 3);
}
...
export default function ViewerShell(): JSX.Element {
  ...
  const [animationSpeed] = useState(readAnimationSpeedFromUrl);
  const [exportCanvasMode] = useState(readExportCanvasModeFromUrl);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const elapsedSeconds = (now / 1000) * animationSpeed;
      const nextValues = new Map<string, number>();
  ...
  }, [animationSpeed, applyJointValues, renderOptions.autoAnimate, urdfSpec]);

  useEffect(() => {
    if (!exportCanvasMode || !urdfSpec) {
      if (typeof window !== "undefined") {
        delete window.__ARTICRAFT_EXPORT__;
      }
      return;
    }

    const exportJoints = buildPreviewJointSequence(urdfSpec);
    const joints = exportJoints.map((joint) => ({
      name: joint.name,
      type: joint.type,
    }));
    const jointByName = new Map(exportJoints.map((joint) => [joint.name, joint]));

    window.__ARTICRAFT_EXPORT__ = {
      ready: true,
      joints,
      setSingleJointPhase: (jointName: string, phase: number) => {
        const joint = jointByName.get(jointName);
        if (!joint) {
          return;
        }
        applyJointValues(new Map([[joint.name, exportJointValue(joint, phase)]]));
        viewportInvalidateRef.current?.();
      },
      setSequentialJointPhase: (jointIndex: number, phase: number) => {
        const clampedIndex = THREE.MathUtils.clamp(
          Math.trunc(jointIndex),
          0,
          Math.max(exportJoints.length - 1, 0),
        );
        const nextValues = new Map<string, number>();

        exportJoints.forEach((joint, index) => {
          if (index < clampedIndex) {
            nextValues.set(joint.name, exportJointValue(joint, 1));
          } else if (index === clampedIndex) {
            nextValues.set(joint.name, exportJointValue(joint, phase));
          }
        });
  ...
  }, [applyJointValues, exportCanvasMode, urdfSpec]);
    ...
    return (
      <div className="relative min-h-0 flex-1 bg-[var(--surface-2)]">
        <ViewportPanel
          ...
          disabledOverlay={hydrationOverlay ?? (missingArtifactsState ? (
