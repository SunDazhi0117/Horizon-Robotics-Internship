...
}): Promise<GenerateFromPhotoResult> {
  return fetchJson<GenerateFromPhotoResult>("/api/generate/from-photo", {
    method: "POST",
    headers: {
...
export async function saveFeedbackMemory(params: {
  ...
  fix: string;
  tags: string[];
}): Promise<FeedbackMemoryResponse> {
  return fetchJson<FeedbackMemoryResponse>("/api/feedback-memory", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function fetchFeedbackMemory(limit = 50): Promise<FeedbackMemoryEntry[]> {
  return fetchJson<FeedbackMemoryEntry[]>(`/api/feedback-memory?limit=${encodeURIComponent(String(limit))}`);
}
...
export async function exportRecordMp4(
  recordId: string,
  params: { viewerUrl?: string | null } = {},
): Promise<ExportMp4Result> {
  return fetchJson<ExportMp4Result>(`/api/records/${encodeURIComponent(recordId)}/export-mp4`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function exportStagingMp4(
  runId: string,
  recordId: string,
  params: { viewerUrl?: string | null } = {},
): Promise<ExportMp4Result> {
  return fetchJson<ExportMp4Result>(
    `/api/staging/${encodeURIComponent(runId)}/${encodeURIComponent(recordId)}/export-mp4`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
