...
def test_generate_from_photo_accepts_multiple_reference_images(
    tmp_path: Path,
    ...
    def fake_popen(cmd: list[str], **_kwargs: object) -> DummyProcess:
        captured["cmd"] = cmd
        return DummyProcess()

    def fake_run(cmd: list[str], **_kwargs: object) -> subprocess.CompletedProcess[str]:
        captured["enhance_cmd"] = cmd
        captured["enhance_input"] = _kwargs.get("input")
        output_path = Path(cmd[cmd.index("--output-last-message") + 1])
        output_path.write_text(
            json.dumps(
                {
                    "prompt": (
                        "Create a compact hinged case based on the reference photos. Use the "
                        "first image as the closed front view and the second image as the open "
                        "state. Model a fixed lower shell, a moving lid, a visible rear hinge "
                        "axis, rounded glossy plastic surfaces, and a clear 0 to 110 degree "
                        "opening range."
                    )
                }
            ),
    ...
    monkeypatch.setattr("viewer.api.photo_generate.subprocess.Popen", fake_popen)
    monkeypatch.setattr("viewer.api.photo_generate.subprocess.run", fake_run)

    client = TestClient(create_app(repo_root=tmp_path))
    feedback_response = client.post(
        "/api/feedback-memory",
        json={
            "record_id": "rec_prior_airpods",
            "object_type": "earbud charging case",
            "issue_types": ["motion", "collision"],
            "problem": "Earbuds clipped through the charging wells when lifted out.",
            "fix": "Use an upward-outward-forward prismatic extraction path with clearance.",
            "tags": ["earbud", "case", "lift"],
        },
    )
    ...
    response = client.post(
        ...
        json={
            "prompt": "Photo 1 is the front view. Photo 2 shows the lid open.",
            "images": [
                {
                    "image_data": base64.b64encode(b"front").decode("ascii"),
                    "image_filename": "front.jpg",
                    "image_content_type": "image/jpeg",
                },
                {
                    "image_data": base64.b64encode(b"open").decode("ascii"),
                    "image_filename": "open.png",
    ...
    assert Path(payload["image_paths"][0]).name == "reference_01.jpg"
    assert Path(payload["image_paths"][1]).name == "reference_02.png"
    assert Path(payload["enhanced_prompt_path"]).name == "enhanced_prompt.txt"
    assert captured["enhance_cmd"].count("--image") == 2
    assert "upward-outward-forward prismatic extraction path" in str(captured["enhance_input"])
    assert Path(payload["prompt_memory_path"]).name == "prompt_memory.txt"
    assert captured["cmd"].count("--image") == 2
    prompt_text = Path(payload["prompt_path"]).read_text(encoding="utf-8")
    assert "2 attached real-world reference photos" in prompt_text
    assert "visible rear hinge axis" in prompt_text
