...
def gpu_worker_main(
    ...
    # SAM3D checkpoints are ~15GB, so serialize their loading to avoid severe
    # disk contention. Hunyuan3D Mini is much smaller and benefits from loading
    # concurrently when fast preview uses several GPU workers.
    if preload_pipeline:
        serialize_preload = backend == "sam3d" and init_lock is not None
        if serialize_preload:
