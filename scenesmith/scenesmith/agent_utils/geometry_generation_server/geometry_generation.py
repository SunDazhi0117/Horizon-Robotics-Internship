...
def _read_int_env(
    ...
    *,
    ...
    """Read a bounded integer setting without changing default behavior."""
    ...
    try:
        ...
        raise ValueError(f"{name} must be an integer, got {raw_value!r}") from e
...
def _generate_with_hunyuan3d(
    ...
    inference_steps = _read_int_env(
        "SCENESMITH_HUNYUAN_INFERENCE_STEPS",
        5,
        minimum=1,
        maximum=50,
    )
    octree_resolution = _read_int_env(
        "SCENESMITH_HUNYUAN_OCTREE_RESOLUTION",
        256,
        minimum=64,
        maximum=512,
    )
    num_chunks = _read_int_env(
        "SCENESMITH_HUNYUAN_NUM_CHUNKS",
        8000,
        minimum=100,
        maximum=100000,
    )
    console_logger.info(
        "Hunyuan3D generation settings: steps=%d, octree_resolution=%d, "
        "num_chunks=%d",
        inference_steps,
        octree_resolution,
        num_chunks,
    ...
    # Apply texture when the full Hunyuan pipeline is enabled.
