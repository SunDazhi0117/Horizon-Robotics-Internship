...
class LocalSmokeImageGenerator(BaseImageGenerator):
    """Create simple local reference images for dependency smoke tests."""
    ...
    def generate_images(
        ...
        del style_prompt, size, labels
        if len(object_descriptions) != len(output_paths):
            raise ValueError("Number of descriptions must match number of output paths")
        for description, output_path in zip(object_descriptions, output_paths):
            self._draw_object(description, output_path)
            console_logger.info(f"Created local smoke image for {description}")
    ...
    def generate_furniture_context_image(
        ...
        del scene_description, width_m, length_m
        return self._copy_image(reference_image_path, output_path)
    ...
    @staticmethod
    def _copy_image(reference_image_path: Path, output_path: Path) -> Path:
    ...
    @staticmethod
    def _draw_object(description: str, output_path: Path) -> None:
        image = Image.new("RGB", (1024, 1024), "#f7f7f5")
        shadow = Image.new("RGBA", image.size, (0, 0, 0, 0))
        shadow_draw = ImageDraw.Draw(shadow)
        shadow_draw.ellipse((190, 745, 835, 875), fill=(0, 0, 0, 55))
        ...
        if "chair" in text:
            LocalSmokeImageGenerator._draw_chair(draw)
        elif any(word in text for word in ("shelf", "bookcase", "storage")):
            low_wide = any(
                phrase in text
                for phrase in ("low wide", "low horizontal", "storage console")
            )
            LocalSmokeImageGenerator._draw_shelf(draw, low_wide=low_wide)
        elif any(word in text for word in ("counter", "cabinet", "drawer")):
            LocalSmokeImageGenerator._draw_counter(draw)
        elif any(word in text for word in ("desk", "table")):
            LocalSmokeImageGenerator._draw_desk(draw)
        else:
            LocalSmokeImageGenerator._draw_counter(draw)
    ...
    def _draw_desk(draw: ImageDraw.ImageDraw) -> None:
        ...
        wood = "#b98555"
        dark = "#855c39"
        draw.polygon([(205, 390), (690, 330), (825, 440), (325, 510)], fill=wood, outline=edge, width=8)
        draw.polygon([(325, 510), (825, 440), (825, 505), (325, 578)], fill=dark, outline=edge, width=7)
        for x, y in ((350, 565), (770, 505), (250, 500), (720, 445)):
            draw.polygon([(x, y), (x + 38, y - 4), (x + 24, 805), (x - 6, 810)], fill="#594435", outline=edge, width=6)
    ...
    def _draw_chair(draw: ImageDraw.ImageDraw) -> None:
        ...
        fabric = "#5f7889"
        dark = "#435764"
        draw.polygon([(330, 220), (635, 255), (645, 560), (345, 525)], fill=fabric, outline=edge, width=9)
        draw.polygon([(305, 530), (620, 565), (730, 655), (395, 620)], fill=dark, outline=edge, width=9)
        for x, y in ((400, 615), (680, 650), (330, 575), (590, 610)):
            draw.line((x, y, x - 22, 820), fill=edge, width=24)

    @staticmethod
    def _draw_shelf(draw: ImageDraw.ImageDraw, low_wide: bool = False) -> None:
        edge = "#343330"
        wood = "#a9784c"
        if low_wide:
            draw.rectangle((175, 380, 850, 755), fill=wood, outline=edge, width=10)
            draw.rectangle((225, 430, 800, 690), fill="#ece5db", outline=edge, width=7)
            draw.rectangle((205, 545, 820, 575), fill="#8a603e", outline=edge, width=6)
            draw.line((415, 430, 415, 690), fill=edge, width=7)
            draw.line((610, 430, 610, 690), fill=edge, width=7)
            draw.rectangle((195, 690, 830, 765), fill="#8a603e", outline=edge, width=7)
        else:
            draw.rectangle((285, 185, 735, 820), fill=wood, outline=edge, width=10)
            draw.rectangle((335, 235, 685, 775), fill="#ece5db", outline=edge, width=7)
            for y in (365, 500, 635):
                draw.rectangle((320, y, 700, y + 28), fill="#8a603e", outline=edge, width=6)
            draw.line((510, 235, 510, 775), fill=edge, width=7)

    @staticmethod
    def _draw_counter(draw: ImageDraw.ImageDraw) -> None:
        edge = "#343536"
        body = "#c9c4b8"
        side = "#969187"
        top = "#55585b"
        draw.polygon([(235, 375), (705, 330), (810, 420), (330, 475)], fill=top, outline=edge, width=9)
        draw.polygon([(330, 475), (810, 420), (805, 790), (330, 840)], fill=body, outline=edge, width=9)
        draw.polygon([(235, 375), (330, 475), (330, 840), (240, 735)], fill=side, outline=edge, width=8)
        draw.line((565, 450, 565, 815), fill=edge, width=7)
        draw.ellipse((510, 610, 530, 635), fill=edge)
        draw.ellipse((600, 600, 620, 625), fill=edge)
...
def create_image_generator(backend: str, config: DictConfig) -> BaseImageGenerator:
    ...
    elif backend == "local-smoke":
        return LocalSmokeImageGenerator()
    else:
