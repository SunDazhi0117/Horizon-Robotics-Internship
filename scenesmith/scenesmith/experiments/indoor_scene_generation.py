...
class IndoorSceneGenerationExperiment(BaseExperiment):
    ...
    def _start_geometry_server(self) -> None:
        ...
        if backend == "sam3d":
            ...
            hunyuan3d_cfg = self.cfg.furniture_agent.asset_manager.get("hunyuan3d", {})
            use_mini = bool(hunyuan3d_cfg.get("use_mini", False))
    ...
    def _start_materials_server(self) -> None:
        ...
        if not self.cfg.floor_plan_agent.materials.use_retrieval_server:
            console_logger.info("Materials retrieval server disabled by configuration")
            return
    ...
    def generate_scenes(self) -> None:
        ...
        try:
            floor_plan_only = start_stage == stop_stage == "floor_plan"
            if not floor_plan_only:
